...
# ===================== USER CUSTOM EMOJI ORDER v2.12 =====================
# The user-defined numbering is authoritative: (1) -> first ID, ..., (83) -> 83rd ID.
# Duplicate IDs are intentionally preserved at their numbered positions.
USER_CUSTOM_EMOJI_IDS = [
    5472055112702629499,
    5260293700088511294,
    6071317996704370355,
    5334544901428229844,
...
# Обычные emoji, которые встречаются в старых строках кода, автоматически
# переводим в соответствующие Premium Emoji из списка владельца. Это нужно,
# чтобы даже старые/динамические сообщения не отправлялись с обычными emoji.
VISIBLE_EMOJI_POSITION = {
    "👋": 1, "⛔": 2, "⏳": 3, "ℹ️": 4, "✅": 5, "🚨": 6, "⚠️": 7,
...
def premiumize_text(text: str | None) -> str | None:
    if text is None or not isinstance(text, str):
        return text
    # Не трогаем уже корректно вставленные tg-emoji, иначе получится вложенный HTML.
    ...
    for i in range(0, len(parts), 2):
        ...
        for emoji, position in sorted(VISIBLE_EMOJI_POSITION.items(), key=lambda x: len(x[0]), reverse=True):
            if not position:
                continue
            replacement = custom_emoji_position(position, emoji)
            value = value.replace(emoji, replacement)
        parts[i] = value
    return ''.join(parts)


# Глобальная страховка: все сообщения, отправляемые через Bot.send_message/edit_message_text,
# проходят через premiumize_text. Поэтому Premium Emoji работают не только в новых
# функциях, но и в старых обработчиках, уведомлениях и сообщениях в разных чатах.
...
PREMIUM_EMOJI_IDS = {slot: SCREEN_CUSTOM_EMOJI_IDS[pos] for slot, pos in SLOT_POSITION.items() if pos in SCREEN_CUSTOM_EMOJI_IDS}
...
async def announce_bot_version() -> None:
    ...
    text = (
        ...
        f"{await custom_emoji_html("calendar", "📅")} Дата обновления: <b>{date_text}</b>\n\n"
